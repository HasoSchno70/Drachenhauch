...
- **[Code-Editor](docs/editor.md)** — Tastenkürzel, Snippets, Minimap, Multi-Cursor, Sidebar, Run/Bench, Signature-Help, **Breadcrumbs** (Scope-Pfad), **Peek-Definition** (Alt+F12), **Debugger** (Breakpoints inkl. **bedingter** Breakpoints/Step/Variablen), **Profiler** (Hotpath pro Zeile/Funktion), **Git-Blame**-Panel, Welcome-Showcase (Demo-Galerie mit Screenshots)
...
- **[Tracker](docs/tracker.md)** — mehrspuriger Chiptune-Musik-Editor (`gbtracker`): 3 Ton-Kanäle + Drums, mehrere Patterns einstellbarer Länge + Song-Arrangement, **Lautstärke pro Note** (Effekt-Spalte), Projekt-Speichern/Laden (`.json`), Export als frame-basierter GB-Player
