...
BUILTIN_DOCS: dict[str, tuple[str, str]] = {
    ...
    "buffer_get":    ("BUFFER_GET(puffer, position) AS INTEGER", "Byte 0..255 an dieser Stelle."),
    "buffer_set":    ("BUFFER_SET(puffer, position, byte)", "Byte 0..255 setzen (veraendert IN PLACE)."),
    "buffer_fill":   ("BUFFER_FILL(puffer, byte)", "Ganzen Puffer mit einem Byte fuellen."),
    "buffer_resize": ("BUFFER_RESIZE(puffer, groesse)", "Groesse aendern; waechst mit Nullen, schrumpft durch Abschneiden."),
    "buffer_slice":  ("BUFFER_SLICE(puffer, von, bis) AS BUFFER",
                      "Neuer Puffer mit den Bytes [von, bis). Klemmt an den Grenzen (wie Array-Slicing)."),
    "buffer_concat": ("BUFFER_CONCAT(a, b) AS BUFFER", "Neuer Puffer aus beiden hintereinander."),
    "buffer_indexof": ("BUFFER_INDEXOF(puffer, nadel[, ab]) AS INTEGER",
                       "Position der ersten Fundstelle, sonst -1."),
    "buffer_from_string": ("BUFFER_FROM_STRING(text) AS BUFFER", "Text als UTF-8-Bytes."),
    "buffer_to_string$":  ("BUFFER_TO_STRING$(puffer) AS STRING",
                           "Bytes als UTF-8-Text. Fehler, wenn es kein gueltiges UTF-8 ist "
                           "(fuer Binaerdaten BUFFER_TO_HEX$ nehmen)."),
    "buffer_to_hex$":     ("BUFFER_TO_HEX$(puffer) AS STRING", "Bytes als Hex-Text ('deadbeef')."),
    "buffer_from_hex":    ("BUFFER_FROM_HEX(hex) AS BUFFER", "Hex-Text zu Bytes; Leerzeichen sind erlaubt."),
    "buffer_to_base64$":  ("BUFFER_TO_BASE64$(puffer) AS STRING", "Bytes als Base64."),
    "buffer_from_base64": ("BUFFER_FROM_BASE64(text) AS BUFFER",
                           "Base64 zu rohen Bytes (anders als BASE64_DECODE, das UTF-8 verlangt)."),
    "buffer_get_i16": ("BUFFER_GET_I16(puffer, position[, reihenfolge]) AS INTEGER", "16-Bit mit Vorzeichen lesen."),
    "buffer_get_u16": ("BUFFER_GET_U16(puffer, position[, reihenfolge]) AS INTEGER", "16-Bit ohne Vorzeichen lesen."),
    "buffer_get_i32": ("BUFFER_GET_I32(puffer, position[, reihenfolge]) AS INTEGER", "32-Bit mit Vorzeichen lesen."),
    "buffer_get_u32": ("BUFFER_GET_U32(puffer, position[, reihenfolge]) AS INTEGER", "32-Bit ohne Vorzeichen lesen."),
    "buffer_get_i64": ("BUFFER_GET_I64(puffer, position[, reihenfolge]) AS INTEGER", "64-Bit mit Vorzeichen lesen."),
    "buffer_get_f32": ("BUFFER_GET_F32(puffer, position[, reihenfolge]) AS FLOAT", "32-Bit-Gleitkomma lesen."),
    "buffer_get_f64": ("BUFFER_GET_F64(puffer, position[, reihenfolge]) AS FLOAT", "64-Bit-Gleitkomma lesen."),
    "buffer_set_i16": ("BUFFER_SET_I16(puffer, position, wert[, reihenfolge])", "16-Bit mit Vorzeichen schreiben."),
    "buffer_set_u16": ("BUFFER_SET_U16(puffer, position, wert[, reihenfolge])", "16-Bit ohne Vorzeichen schreiben."),
    "buffer_set_i32": ("BUFFER_SET_I32(puffer, position, wert[, reihenfolge])", "32-Bit mit Vorzeichen schreiben."),
    "buffer_set_u32": ("BUFFER_SET_U32(puffer, position, wert[, reihenfolge])", "32-Bit ohne Vorzeichen schreiben."),
    "buffer_set_i64": ("BUFFER_SET_I64(puffer, position, wert[, reihenfolge])", "64-Bit mit Vorzeichen schreiben."),
    "buffer_set_f32": ("BUFFER_SET_F32(puffer, position, wert[, reihenfolge])", "32-Bit-Gleitkomma schreiben."),
    "buffer_set_f64": ("BUFFER_SET_F64(puffer, position, wert[, reihenfolge])", "64-Bit-Gleitkomma schreiben."),
    # Binaerdateien (WP B) -- dieselben FILE-Handles wie READLINE/WRITELINE
    "readall_bytes":  ("READALL_BYTES(pfad) AS BUFFER", "Ganze Datei als Bytes lesen."),
    "writeall_bytes": ("WRITEALL_BYTES(pfad, puffer)", "Bytes in eine Datei schreiben (ueberschreibt)."),
    "read_bytes":     ("READ_BYTES(datei, anzahl) AS BUFFER",
                       "Bis zu `anzahl` Bytes lesen. Am Dateiende kommen WENIGER zurueck "
                       "(0 = fertig) -- das ist kein Fehler."),
    "write_bytes":    ("WRITE_BYTES(datei, puffer)", "Bytes an die geoeffnete Datei schreiben."),
    "seek":           ("SEEK(datei, position)", "Lese-/Schreibposition auf ein Byte setzen (0 = Anfang)."),
    "tell":           ("TELL(datei) AS INTEGER", "Aktuelle Lese-/Schreibposition."),
