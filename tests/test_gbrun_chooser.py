"""gbrun: ohne Argumente erscheint der Start-Dialog (Code-Editor / Form-Designer).
...
def test_no_args_routes_to_chooser(monkeypatch):
    called = {}
    monkeypatch.setattr(gbrun, "_launch_chooser",
                        lambda root: called.setdefault("x", True) and 0 or 0)
    rc = gbrun.main(["gbrun.py"])
...
def test_file_arg_does_not_open_chooser(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gbrun, "_run_native", lambda abs_path, path: 0)
    f = tmp_path / "x.gb"
    ...
    cwd = os.getcwd()
    try:
        assert gbrun.main(["gbrun.py", str(f)]) == 0
    finally:
        os.chdir(cwd)
...
# decken die gbrun-Verdrahtung ab.
