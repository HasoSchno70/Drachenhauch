...
vergleicht die Werte. Driftet eine Seite (Konstante fehlt in gbrt oder hat
einen anderen Wert), schlaegt er mit dem Namen fehl. Die Gegenrichtung
(zusaetzliche Konstanten nur in gbrt) deckt er bewusst nicht ab -- Python
...
def test_colors_match_runtime(run_gb):
    ...
    for name, got in zip(names, out):
        assert got == str(COLORS[name]), (
            f"Farbe {name.upper()}: graphics.py={COLORS[name]}, gbrt={got} "
...
def test_keys_match_runtime(run_gb):
    ...
    for name, got in zip(names, out):
        assert got == str(KEYS[name]), (
            f"Taste {name.upper()}: graphics.py={KEYS[name]}, gbrt={got} "
