...
def test_dim_external_type_without_import_hints_import(run_gb):
    with pytest.raises(DrachenhauchError, match=r'Unbekannter Typ .vec2.*IMPORT "vec2"'):
...
def test_dim_external_type_shared_lists_candidates(run_gb):
    ...
    with pytest.raises(DrachenhauchError, match=r'Unbekannter Typ .tiled_map.*IMPORT'):
        run_gb("DIM m AS TILED_MAP\n")


def test_dim_truly_unknown_type(run_gb):
    with pytest.raises(DrachenhauchError, match=r"Unbekannter Typ .foobar."):
