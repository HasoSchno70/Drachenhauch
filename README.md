...
- **[Web-Playground](docs/web-playground.md)** — `gbrt` als WebAssembly im Browser: Quelle im `<textarea>` tippen → gbrt kompiliert **im Browser** (kein Pyodide) → **Konsole UND animierte Grafik im `<canvas>`** (Render-Loop yieldet pro Frame, kein Tab-Freeze). **Teilbare Links** (Quelle im URL-Hash → öffnen = sehen + starten). **Assets** (`assets/` neben der `.gb`) kommen als `gbrt.data` mit — Bilder, Schriften, Musik unter demselben Pfad wie auf dem Desktop. **Ton läuft stumm über Kiras MockBackend**: nicht hörbar, aber Abspielposition, Kanäle und `AUDIO_FFT` stimmen — damit läuft sogar die musikgetriebene `gbdemo/` im Browser. **3D läuft mit** — der Web-Build fährt WebGL 2, dessen GLSL ES 3.00 bis auf den Kopf identisch zu unserem Desktop-GLSL ist: PBR, HDR-IBL, Skybox, Schatten, Instancing und Post-Effekte sind im Browser verifiziert. Build `rust/build_wasm.py`, Harness `web/`
