...
| `audio` | Erweiterte Audio-API ueber pygame.mixer. Channels, Pause/Resume/Fade, Stereo-Pan, Music-Position. Tone-Generation (`AUDIO_TONE`/`AUDIO_NOISE`) mit Sine/Square/Saw/Triangle/Noise. **`AUDIO_SFX`** -- prozeduraler sfxr-Stil-Synth (Waveform + Pitch-Slide + ADSR + Vibrato; geteilte Mathematik in `gamebasic/synth.py`, nativ in `rust/gb_runtime/src/audio.rs`; der SFX-Generator `gbsfx` exportiert solche Aufrufe). Liefert kompatible `SOUND`-Objekte (auch fuer `PLAYSOUND` nutzbar). | `AUDIO_CHANNEL` |
