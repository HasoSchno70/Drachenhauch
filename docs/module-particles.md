...
`PARTICLE_DRAW` berechnet beim Fade-Effekt die per-Partikel-Farben vorab in einer einzigen vektorisierten Operation und ruft dann pro Partikel ein `pygame.draw.circle()`.

Voraussetzung: `numpy` (wird über `pip install numpy` geholt; bei den meisten Pygame-Setups ohnehin installiert).

## Komplettes Beispiel

- [examples/28_particles.gb](../examples/28_particles.gb) — Konsolen-Logik-Test mit `RANDOMIZE(42)`
