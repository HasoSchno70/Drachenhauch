...
> ausschließlich `dhrt` (`rust/gb_runtime/src/`: `lexer.rs`/`parser.rs`/
...
**Module sind in dhrt/Rust implementiert** (`rust/gb_runtime/src/<modul>.rs` +
...
| 2D-Extras | **Nativ in dhrt:** `LINEW(x1,y1,x2,y2,breite[,c])` (dicke Linie), `BOXROUND`/`RECTROUND(x1,y1,x2,y2,radius[,c])` (runde Rechtecke gefuellt/Umriss), `GRADIENTV`/`GRADIENTH(x1,y1,x2,y2,c1,c2)` (Farbverlauf-Blocks), `SPLINE(xs,ys[,c[,breite]])` (Catmull-Rom durch Punkte). Demo `examples/100_2d_extras.gb`. | — |
...
```
Baut `rust/gb_runtime/` → `dhrt`. Nötig für Run/Export/Editor-Run + die Tests
...
**Headless prüfen:** `DHRT_FRAMES=n DHRT_SCREENSHOT=p.png dhrt run x.gb` liefert
...
**Kontaktbogen**: `DHRT_FRAMES=480 DHRT_CONTACT=bogen.png dhrt run x.gb` setzt
...
- **Neue Builtins/Sprach-Features NUR in dhrt** (`rust/gb_runtime/src/`):
...
- **`run_gb`/`run_vm`/`run_native`/`run_all`-Fixtures** sind alle Aliase auf
  `dhrt run` (conftest); `run_gb(src, base=tmp_path)` legt die .gb in ein
...
`dhrt` (Rust). Kein `setup.py build_ext` mehr, kein neuer Cython-Code.
...
Toolchain; der Rest läuft in `dhrt`. (Tree-Walker + Python-Compiler selbst zu
...
Verdrahtung (5). **`dhrt run datei.gb` ist ein eigenständiger End-to-End-Lauf
ohne Python** (preprocess→lex→parse→compile→VM, chdir ins Datei-Verzeichnis für
relative Asset-/IMPORT-Pfade; `dhrt datei.gb` ohne `run` per `.gb`-Auto-Detect
...
schaltet Preprocess vor. **Stufe 5 (Verdrahtung) fertig:** `dhrt run datei.gb`
(+ `dhrt datei.gb` per `.gb`-Auto-Detect) — eigenständiger End-to-End-Lauf ohne
...
(a) **Selbst-Export** `dhrt --export datei.gb [out]` — kompiliert Quelle → `.gbc`,
