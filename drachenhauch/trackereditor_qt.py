...
(nutzt den geteilten Synth `drachenhauch.synth`), Projekt als `.json` speichern/
laden und als GB-Code exportieren -- ein frame-basierter Player
(`TRACKER_UPDATE`), der mit `DELTA()` im Game-Loop laeuft.

Das Datenmodell + I/O + GB-Export liegen Qt-frei in `drachenhauch.tracker`
(headless getestet).
"""
...
class TrackerEditor(QMainWindow):
    def __init__(self, project_root: Path):
        ...
        self._mixer = Mixer()          # additiver Mixer (drachenhauch.audio_preview)
    ...
    def _export(self) -> None:
        if self.song.has_gb_code_fidelity_gaps():
            QMessageBox.information(
                ...
                "GB-Code verwenden.")
        self._show_code(self.song.dh_code())
