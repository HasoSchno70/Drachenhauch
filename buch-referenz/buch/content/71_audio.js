module.exports = (H) => [
  ...
  H.cmd("SAMPLE_LOAD · SAMPLE_PLAY · SAMPLE_SET_LOOP · SAMPLE_LEN", 'SAMPLE_LOAD(pfad$)   SAMPLE_PLAY(sample, halbtoene, vol [, dur_ms])   SAMPLE_SET_LOOP(sample, start, ende [, art$])   SAMPLE_LEN(sample)',
    "LOAD lädt eine Audiodatei als SAMPLE. PLAY spielt sie um halbtoene transponiert (0 = Originaltonhöhe, 12 = eine Oktave höher) und gibt eine Kanal-Nummer zurück. SET_LOOP markiert eine Schleifenregion in Frames; mit \"pingpong\" läuft sie am Ende rückwärts wieder hinunter, statt an ihren Anfang zu springen. LEN liefert die Länge in Sekunden.",
    [
      'IMPORT "audio"',
      'DIM piano AS SAMPLE',
  ...
  H.cmd("SAMPLE_FROM_BUFFER · SAMPLE_NOTE", 'SAMPLE_FROM_BUFFER(puffer, abtastrate)   SAMPLE_NOTE(sample, halbtoene, dauer_ms, attack_ms, decay_ms, sustain, release_ms, vol [, slide_halbtoene])',
    "FROM_BUFFER macht ein Sample aus rohen 16-Bit-Werten (little endian, mit Vorzeichen, mono) – etwa aus Base64-Daten in einer JSON-Datei, ohne Umweg über eine WAV. NOTE spielt nichts ab, sondern baut einen Klang (SOUND) mit derselben Hüllkurve wie AUDIO_NOTE: den kann man mit AUDIO_PLAY_AT auf eine Audio-Uhr legen oder mit AUDIO_SOUND_MIX in einen anderen mischen. So spielt der Tracker seine Sample-Instrumente.",
    [
      'IMPORT "audio"',
      'IMPORT "json"',
      'DIM inst AS JSON_HANDLE',
      'inst = JSON_LOAD("instrument.json")',
      'DIM s AS SAMPLE',
      's = SAMPLE_FROM_BUFFER(BUFFER_FROM_BASE64(JSON_GET_STRING(inst, "samples")), 22050)',
      'SAMPLE_SET_LOOP(s, 1100, 2200, "pingpong")',
      'DIM ton AS SOUND',
      "ton = SAMPLE_NOTE(s, 7, 500, 5, 100, 0.6, 200, 0.8)   ' Quinte höher",
      'AUDIO_PLAY(ton)',
    ]),
