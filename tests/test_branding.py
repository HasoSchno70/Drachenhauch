...
def test_smart_square_crop_already_square_returns_original():
    ...
    from drachenhauch.branding import _smart_square_crop
...
def test_is_available_with_logo_present():
    ...
    pytest.importorskip("PIL")
    from drachenhauch.branding import is_available, _logo_path
...
def test_scaled_logo_image_scales_proportionally():
    ...
    target_w)."""
    pytest.importorskip("PIL")
    from drachenhauch.branding import scaled_logo_image, is_available, _logo_path
    if not _logo_path().exists():
        pytest.skip("Logo-Datei nicht im Repo")
    assert is_available()
...
def test_scaled_logo_image_none_when_unavailable(monkeypatch):
    import drachenhauch.branding as branding
