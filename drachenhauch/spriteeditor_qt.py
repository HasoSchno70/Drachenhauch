...
# `from drachenhauch.spriteeditor_qt import SpriteDoc` importiert.
...
class SpriteEditorWindow(QMainWindow):
    ...
    def _recent_path() -> Path:
        try:
            d = Path.home() / ".drachenhauch"
            d.mkdir(exist_ok=True)
            return d / "recent_sprites.json"
        except Exception:
            return Path.home() / ".drachenhauch_sprites_recent.json"
    ...
    def action_test_sprite(self):
        ...
        anzeigt (animiert wenn Multi-Frame), und startet es via dhrun.py
        als Subprozess. Der User sieht das Sprite live im
        nativen dhrt-Fenster wie es im Spiel aussehen wuerde.
        """
        ...
        if not dhrun.exists():
            ...
            return
        # Temp-Verzeichnis -- bewusst NICHT loeschen, der dhrun-Subprozess
