...
def line_color_spans(text: str) -> list[tuple[int, int, str | None]]:
    ...
    comment_start = DHHighlighter._find_comment_start(text)
    ...
    try:
        ...
        tokens = []
    for tok in tokens:
        start, length = DHHighlighter._token_span(text, tok)
        if length <= 0:
            continue
        ...
        for i in range(start, min(start + length, n)):
            keys[i] = key
    for fstart, flen in DHHighlighter._find_fstring_ranges(lex_target):
...
class DHHighlighter(QSyntaxHighlighter):
