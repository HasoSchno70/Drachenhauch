...
Persistente Fenster/Widgets (externe Typen `GUI_WINDOW`/`GUI_WIDGET` via `register_type`). Aufbau einmalig, pro Frame `GUI_UPDATE()` + `GUI_DRAW()`; Events per Polling (`GUI_CLICKED`/`CHECKED`/`VALUE`/`TEXT`/`HOVERED`). Window-Drag an der Titelleiste, Z-Order (Klick bringt nach vorne), Fokus, Schliessen-Button, Cyan-Theme. Konstruktoren/Getter sind `@builtin`, nur `GUI_UPDATE`/`GUI_DRAW` sind `graphics_builtin`. Komplement zum Immediate-Mode-`ui`-Modul. Doku `docs/module-gui.md`, Demo `examples/45_gui.gb`, Tests `tests/test_modules_gui.py`. (Offen: FUNCREF-Callbacks `GUI_ON_*`, `ui`-Immediate-Mode-Fenster `UI_WINDOW_BEGIN/END`.)
