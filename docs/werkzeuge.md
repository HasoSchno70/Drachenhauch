...
| `--- ersetzen ALT` | nur mit `--- programm`: der Text `ALT` (Rest der Kopfzeile) muss **genau einmal** im Programm stehen und wird durch den Block ersetzt; `{sammlung}` und `{fall}` gelten darin. Für einen Dateidialog, den keine Aufnahme erreicht (`FILE_SAVE_DIALOG(…)` → `"raus.dh"`), oder das Ablesen eines Eingabefeldes — ersetzt wird genau dieser Aufruf, alles dahinter bleibt der echte Code. Mehrere Blöcke erlaubt; zweimal oder keinmal vorhanden ist ein Fehler |
| `--- einschub nach MARKE` | nur mit `--- programm`: ein **weiterer** Einschub hinter der ersten Zeile `MARKE` (mehrere erlaubt, nach dem Quelltext des Falls eingesetzt). Typisch steht der Quelltext oben (Hilfsfunktionen, `DIM`) und ein Einschub hinter `FLIP()` misst je Bild — erst dort steht die Fenstergeometrie fest, also kann er dort die Aufnahme mit echten Klicklagen schreiben und abspielen |
