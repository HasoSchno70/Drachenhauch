...
> Ausgabevergleiche in 79 Dateien. **60 Dateien mit 1 230 Faellen sind
> umgezogen und aus `tests/` geloescht** -- nicht durch Umbau des Quelltexts,
> sondern durch Aufzeichnung: ein Wegwerf-Plugin schrieb bei zwei
> pytest-Laeufen je Aufruf Quelltext, Ausgabe und Beilagen mit, und nur was
> beide Male gleich war, wurde ein Fall. Bleiben in pytest: alles mit
> Fenster, Ton oder Eingabe (bis `--- bild` da ist), das Dateisystem
> (Dateizeiten, Gross/Klein je System) und 14 Module mit binaeren Beilagen
> oder Zeitabhaengigkeit.
