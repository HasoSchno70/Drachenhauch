...
`gbrt --export` haengt `<gbc><laenge><GBRTPAY1>` an eine Kopie der Runtime.
...
ein blankes `gbrt` ("Verwendung: ... <datei.gbc>").
...
def _exportiere(tmp_path, quelltext):
    ...
    res = subprocess.run([str(_GBRT), "--export", str(gb)], cwd=str(tmp_path),
...
def test_blanke_runtime_bleibt_blank(tmp_path):
    """Die Gegenprobe: ohne Nutzlast darf gbrt sich NICHT als Spiel ausgeben.

    Die Rueckwaertssuche koennte sonst irgendwo im Binaercode etwas finden
    und eine leere .exe fuer ein Bundle halten."""
    kopie = tmp_path / _GBRT.name
    kopie.write_bytes(_GBRT.read_bytes())
    res = _lauf(kopie)
