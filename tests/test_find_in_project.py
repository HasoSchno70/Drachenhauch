...
def test_iter_gb_files_filters(tmp_path):
    (tmp_path / "a.dh").write_text("x", encoding="utf-8")
    (tmp_path / "_skip.dh").write_text("x", encoding="utf-8")
    (tmp_path / "note.txt").write_text("x", encoding="utf-8")
    venv = tmp_path / ".venv"; venv.mkdir()
    (venv / "lib.dh").write_text("x", encoding="utf-8")
...
def test_plan_replacements_from_disk(tmp_path):
    (tmp_path / "a.dh").write_text("PRINT foo\nPRINT foo\n", encoding="utf-8")
    (tmp_path / "b.dh").write_text("DIM x AS INTEGER\n", encoding="utf-8")
...
def test_plan_replacements_no_match_empty_plan(tmp_path):
    (tmp_path / "a.dh").write_text("nothing here\n", encoding="utf-8")
...
def test_plan_replacements_counts_unreadable_files(tmp_path, monkeypatch):
    ...
    (tmp_path / "a.dh").write_text("PRINT foo\n", encoding="utf-8")
    (tmp_path / "b.dh").write_text("PRINT foo\n", encoding="utf-8")
    pat = build_pattern("foo", case_sensitive=True, whole_word=False, regex=False)
    ...
    plan, total, skipped = plan_replacements(tmp_path, pat, replacement_string("bar", False))
    assert skipped == 1
    assert total == 1 and len(plan) == 1   # a.dh weiterhin gefunden
...
def test_dialog_replace_all_writes_disk(tmp_path, monkeypatch):
    ...
    assert applied == ["a.dh"]
    assert (tmp_path / "a.dh").read_text(encoding="utf-8") == "PRINT bar\nPRINT bar\n"
    assert (tmp_path / "b.dh").read_text(encoding="utf-8") == "DIM x AS INTEGER\n"
