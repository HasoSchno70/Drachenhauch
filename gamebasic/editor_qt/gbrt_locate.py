...
# debugger.py), die `gbrt` als Subprozess AUS EINEM HINTERGRUND-THREAD heraus
...
# GLEICHZEITIG aufgerufen zu werden -- und `find_gbrt()` selbst (`Path.resolve()`
...
# viele `gbrt --check`-Subprozesse gleichzeitig anstossen (per Thread-Dump
# verifiziert: zweistellige Anzahl Threads gleichzeitig in `find_gbrt`/
...
_MAX_CONCURRENT_GBRT_SPAWNS = 1
gbrt_spawn_semaphore = threading.BoundedSemaphore(_MAX_CONCURRENT_GBRT_SPAWNS)
