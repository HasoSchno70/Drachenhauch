...
class DrachenhauchEditor(QMainWindow):
    ...
    def _build_actions(self) -> None:
        ...
        # (`dhrt run`), bei Startproblemen ueber den dhrun.py-Launcher (gleicher
    ...
    def _update_breadcrumbs(self) -> None:
        ...
        scopes = dh_symbols.scope_path(st.editor.get_text(), line)
    ...
    def _goto_definition(self, name: str) -> None:
        ...
        if st is None:
            return
        d = dh_symbols.find_definition(st.editor.get_text(), name)
        if d is None:
    ...
    def _peek_definition(self, name: str) -> None:
        ...
        d = dh_symbols.find_definition(text, name)
        if d is None:
            self.statusBar().showMessage(
                f"Keine Definition fuer '{name}' gefunden.", 3000)
            return
        info = dh_symbols.extract_user_doc(text, name)
        ...
        lines = text.split("\n")
        end_line = d.line
        for sc in dh_symbols.scan_scopes(text):
            if sc.line == d.line:
                end_line = sc.end
                break
    ...
    def _find_references(self, name: str) -> None:
        ...
        if st is None:
            return
        refs = dh_symbols.scan_references(st.editor.get_text(), name)
        if not refs:
    ...
    def _rename_symbol(self, name: str) -> None:
        ...
        if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", new):
            ...
            return
        text = st.editor.get_text()
        refs = dh_symbols.scan_references(text, name)
        if not refs:
            return
    ...
    def _run_active(self) -> None:
        ...
        # Startproblemen ueber den dhrun.py-Launcher (gleicher dhrt, aber chdir
        ...
        if mode:
            ...
            if mode == "native":
                ...
                self.statusBar().showMessage(
                    f"▶ Laeuft: {st.file_path.name} (dhrt via dhrun.py)")
    ...
    def _run_selection(self, snippet: str) -> None:
        ...
        dhrun.py darauf -- erlaubt schnelles Ausprobieren ohne den Buffer
        zu speichern.
