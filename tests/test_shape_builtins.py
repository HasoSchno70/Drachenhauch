...
# --- Direkte Graphics-Tests (ohne Builtin-Layer) ---------------------

def test_graphics_polygon_validates_min_points():
    """polygon() braucht mindestens 3 Punkte."""
    ...
    try:
        with pytest.raises(GBRuntimeError):
            g.polygon([10, 10, 20, 20])  # nur 2 Punkte
    finally:
        g.shutdown()


def test_graphics_polygon_validates_even_count():
    """polygon() braucht eine gerade Anzahl Werte (Paare)."""
    pytest.importorskip("pygame")
    from gamebasic.graphics import Graphics
    from gamebasic.errors import GBRuntimeError
    g = Graphics()
    ...
    try:
        with pytest.raises(GBRuntimeError):
            g.polygon([10, 10, 20, 20, 30])  # 5 Werte
    finally:
        g.shutdown()


def test_graphics_polygon_accepts_gbarray():
    """polygon() funktioniert auch mit _GBArray (kein __iter__-Protokoll,
    deshalb greift Graphics auf das interne .values-Attribut zu)."""
    pytest.importorskip("pygame")
    from gamebasic.graphics import Graphics
    ...
    g.screen(50, 50, "Test")
    try:
        arr = _GBArray("integer", [6], lambda: 0)
        # _GBArray.values ist array.array fuer ARRAY OF INTEGER -- per-Index
        # befuellen statt slice-assign mit Liste.
        for i, v in enumerate([10, 10, 30, 10, 20, 30]):
            arr.values[i] = v
...
def test_pairs_to_points_works_with_gbarray():
    """Direkter Test der internen Helper-Funktion mit _GBArray."""
    from gamebasic.graphics import Graphics
    from gamebasic.interpreter import _GBArray
    arr = _GBArray("integer", [6], lambda: 0)
    for i, v in enumerate([100, 50, 150, 150, 50, 150]):
        arr.values[i] = v
    pts = Graphics._pairs_to_points(arr)
    assert pts == [(100, 50), (150, 150), (50, 150)]


def test_pairs_to_points_rejects_odd_count():
    from gamebasic.graphics import Graphics
    from gamebasic.errors import GBRuntimeError
    with pytest.raises(GBRuntimeError):
        Graphics._pairs_to_points([1, 2, 3])  # 3 Werte (ungerade)


def test_pairs_to_points_rejects_non_int():
    from gamebasic.graphics import Graphics
    from gamebasic.errors import GBRuntimeError
    with pytest.raises(GBRuntimeError):
        Graphics._pairs_to_points([1, "x", 3, 4])
