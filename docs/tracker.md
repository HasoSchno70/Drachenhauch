...
- **Eigene Instrumente** (Zeile **`Bibliothek:`**) — über die Presets hinaus kannst du **eigene Samples** laden (`+ Sample (WAV)...` — WAV/OGG wird über die Klaviatur resampelt, MOD/XM/IT-Stil) oder ein **Keymap** bauen (`+ Keymap...`). Geladene Instrumente erscheinen in den Spur-Sound-Dropdowns und werden im Projekt (`.json`) eingebettet (self-contained).
