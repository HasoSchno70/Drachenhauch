...
|---|---|---|
| Start | Willkommensseite, solange kein Reiter offen ist: oben der Drachenhauch-Schriftzug (aus `daten/bilder/` neben `ide/`; das Logo dort wird auch das Fenstersymbol, ohne die Bilder steht der Titel als Text da), Neu, Öffnen, Beispiele, die zuletzt geöffneten Dateien (Doppelklick), die wichtigsten Kürzel. Vor allem anderen läuft der **Vorspann** (`daten/video/vorspann.mp4`, zehn Sekunden, eingepasst mit schwarzen Rändern): jede Taste und jeder Klick überspringt ihn, das Kreuz beendet die IDE; abschalten mit „Vorspann beim Start“ in den Einstellungen (`vorspann` in der ide.json). Die Sitzung kommt beim nächsten Start wieder (offene Dateien, aktiver Reiter), ebenso Thema und Schriftgröße | |
...
`stack` -- innen zuerst, je `name` und `line` -- und `watches` -- je `expr`
und `value` oder `error`; `watches` als eigenes Ereignis nach
`set-watches`; `set-result`/`set-error`; `eval-result`/`eval-error`;
`output`; `finished`; `error`) und nimmt Kommandos auf stdin, aber nur,
solange es steht (`continue`, `step-over`, `step-into`, `step-out`,
`set-breakpoints`, `set-watches` mit `exprs`, `run-to` mit `line`, `set`
mit `name` und `value` (ein Ausdruck), `eval`, `stop`). Was während des
Laufens kommt, liest es beim nächsten Halt. Beim ersten Halt in Zeile 1
schickt die IDE Haltepunkte und überwachte Ausdrücke und lässt es
weiterlaufen, wenn es Haltepunkte gibt (oder bis zur Marke, wenn sie so
gestartet wurde). Der Profiler
